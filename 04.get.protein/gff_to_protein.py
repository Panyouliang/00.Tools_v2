...
parser.add_argument('-gff',type=str,required=True,help='Please input the GFF annotation files')
parser.add_argument('-ref',type=str,required=True,help='Please input the reference genome')
parser.add_argument('-sp',type=str,required=False, default='output',help='Please input the species name')
parser.add_argument('-IDtype',type=str,required=False,default='T',help='Please input the G|T, "G" represent geneID, "T" represent transcriptID')
...
def main():
    ...
    for k,v in g_mRNA.items():
        ...
        if IDtype == 'T':
            fcds.write('>'+k[0:-1]+'\t'+g2t[k[0:-1]]+'\n')
        fcds.write(v+'\n')

    fcds.close()

    fpep = open(sp+".protein.fasta",'w')
    for k,v in g_mRNA.items():
        if IDtype == 'G':
            fpep.write('>'+g2t[k[0:-1]]+'\n')
        if IDtype == 'T':
            fpep.write('>'+k[0:-1]+'\t'+g2t[k[0:-1]]+'\n')
            #fpep.write('>'+k[0:-1]+'\n')
        ...
        if subseq[-1] in stop_code:
            ...
            if subseq[-1] in code:
                ...
                protein += '*'
        fpep.write(print_lines(protein)+'\n')
