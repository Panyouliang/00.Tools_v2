...
GEMOMA='/ldfssz1/ST_EARTH/P18Z10200N0107/panyouliang/my_work/00.Tools/13.annotation/bin/GeMoMa-1.9.jar'
...
def ERE(bamdir,strand):
    ...
    #qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=80g,num_proc=4', '-binding', 'linear:4', 'ERE.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def DenoiseIntrons(maxintrons):
    ...
    qsubs = subprocess.run(['qsub', '-cwd', '-l', 'vf=100g,num_proc=1', '-P', args.groupID+'_super', '-binding', 'linear:1', '-q', 'st_supermem.q', 'DenoiseI.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def Extractor(homolis):
    ...
    with open(homolis,'r') as f:
        for line in f:
            ...
            qsubs = subprocess.run(['qsub', '-cwd', '-l', 'vf=100g,num_proc=4', '-P', args.groupID+'_super', '-binding', 'linear:4', '-q', 'st_supermem.q', name+'.extract.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def searchhit(ref):
    ...
    for name in os.listdir(currentdir+'/Extractor/'):
        ...
        #qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=60g,num_proc=4', '-binding', 'linear:4', 'mmseqs.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
        qsubs = subprocess.run(['qsub', '-cwd', '-l', 'vf=80g,num_proc=4', '-P', args.groupID+'_super', '-binding', 'linear:4', '-q', 'st_supermem.q', 'mmseqs.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def GeneModelMapper(ref,maxintrons):
    ...
    for name in os.listdir(currentdir+'/Extractor/'):
        ...
        #qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=120g,num_proc=4', '-binding', 'linear:4', 'GeneModelMapper.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def GMMfilter():
    ...
    #qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=120g,num_proc=4', '-binding', 'linear:4', 'Combine_Filter.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def AnnotationFinalizer(ref,name):
    ...
    #qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=120g,num_proc=4', '-binding', 'linear:4', 'Finalizer.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
