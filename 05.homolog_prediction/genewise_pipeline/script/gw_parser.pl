...
my $codon_list = "/hwfssz1/ST_EARTH/P18Z10200N0107/P17Z10200N0101_Metazoa_RNA_Editing/liqiye/PC_PA_UN/bin/project002/bin/genewise_pipeline/standard_genetic_code.txt";
my %codonList;
open IN, $codon_list;
while (<IN>) {
	my @info = split /\s+/;
...
=cut
			...
			push @result, "$query\t$pepLen{$query}\t$pep_align_bg\t$pep_align_ed\t$alignRate\t$strand\t$chr\t$nuc_align_bg\t$nuc_align_ed\t$score\t$identity\t$mut_type\t$pep_mut_bg\t$pep_mut_ed\t$nuc_mut_bg\t$nuc_mut_ed\t$mut_info\n" if $mut_type;
