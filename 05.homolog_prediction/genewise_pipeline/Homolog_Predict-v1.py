...
parser.add_argument('-pep', metavar='pep', type=str, required=True, help='Please input the core files [.fasta,.fa,.fasta.gz,.fa.gz]')
parser.add_argument('-output', metavar='str', type=str, required=True, default='predict', help='Please set the output dir name')
parser.add_argument('-blast_split', metavar='num', type=str, required=True, default=2, help='split numbers for query sequence [2]')
parser.add_argument('-alignRate', metavar='str', type=float, required=False, default=0, help='blast alignRatio threshold [0-1]')
parser.add_argument('-identity', metavar='str', type=int, required=False, default=0, help='blast alignRatio threshold [0-100]')
parser.add_argument('-besthit', metavar='str', type=str, required=True, default='no', help='whether to only use besthit for genewise predictions [yes|no]')
parser.add_argument('-gwis_split', metavar='str', type=str, required=False, default='5', help='split work to run, default=5')
parser.add_argument('-groupID', metavar='group', type=str, required=False, default='P18Z10200N0107', help='groupID for qsub tasks')
...
def callblast():
    ...
    for f in files:
        ...
        if os.path.exists(file_path):
            ...
            run_blast.write('echo "$?: tblastn was run done!"\n')
            run_blast.close()
            os.chmod(args.output+'/blast/'+f+'.blast.sh',0o775)
            os.chdir(args.output+'/blast')
            qsubs = subprocess.run(['qsub', '-clear','-cwd','-q','st.q','-P', args.groupID, '-l', 'vf=2g,p=1', '-binding', 'linear:1', f+'.blast.sh'], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True)
...
def combinehit():
    ...
    subprocess.run(['python', scripts+'/sort.best_hit.py', args.output+'/combine.m8.solar.cor.idAdd'], stdout=open(args.output+'/combine.m8.solar.cor.idAdd.best', 'w'))
...
def callgwis():
    ...
    if args.besthit == 'no':
        subprocess.run(['perl',scripts+'/get_pos.pl',args.output+'/combine.m8.solar.cor.idAdd'],stdout=open(args.output+'/gwis/gwis.pos','w'))

    if args.besthit == 'yes':
        subprocess.run(['perl',scripts+'/get_pos.pl',args.output+'/combine.m8.solar.cor.idAdd.best'],stdout=open(args.output+'/gwis/gwis.pos','w'))
...
def prepare_data():
    ...
    if os.path.exists(args.output+'/split'):
        ...
        subprocess.run(['python',scripts+'/fasta_split_non_dir.py', args.pep, args.blast_split, 'QueryCut', args.output+'/split'])
