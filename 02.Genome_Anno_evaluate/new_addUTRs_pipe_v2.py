    ...
    for chrs in chrslis:
        if chrs in minus_loc.keys():
            if len(minus_loc[chrs]) > 1:
                ...
                if lrpid+' -' in UTR5.keys():
                    splitUTR5[lrpid] = UTR5[lrpid+' -']
                if lrpid+' -' in UTR3.keys():
                ...
                for pnum in range(len(plus_loc[chrs])-1):
                    ...
                    if pabsv > 10:
                        ...
                        if rpid+' +' in UTR5.keys():
                            UTR5[rpid+' +'] = [x for x in UTR5[rpid+' +'] if x > cut_p_value]
...
def main():
    ...
    for k,v in coregff.items():
        for tid,fea in v.items():
            for line in fea:
                if line[2] == 'mRNA':
                    utr3lis = outUTR(line,tid,isoUTR3,ngsUTR3,coreUTR3,'UTR3')
                    if utr3lis is not None:
                        for utr3s in utr3lis:
                            if tid in gene_dff.keys():
                                gene_dff[tid].append(utr3s)
                            else:
                                gene_dff[tid] = [utr3s]

                    utr5lis = outUTR(line,tid,isoUTR5,ngsUTR5,coreUTR5,'UTR5')
                    if utr5lis is not None:
                        for utr5s in utr5lis:
                            if tid in gene_dff.keys():
                                gene_dff[tid].append(utr5s)
                            else:
                                gene_dff[tid] = [utr5s]

                else:
                    if tid in gene_dff.keys():
                        gene_dff[tid].append(line)
                    else:
                        gene_dff[tid] = [line]

    gene_range = {}
    for k,v in gene_dff.items():
        gene_range[k] = []
        if v[0][6] == '-':
            v.sort(key=lambda x:int(x[3]),reverse=True)
            for line in v:
                gene_range[k].extend([int(line[3]),int(line[4])])
        else:
            v.sort(key=lambda x:int(x[3]),reverse=False)
            for line in v:
                gene_range[k].extend([int(line[3]),int(line[4])])

    for gid,trans in coregff.items():
        for tid,v in trans.items():
            mRNA_tag = v[0].copy()
            mRNA_tag[3] = str(min(gene_range[tid]))
            mRNA_tag[4] = str(max(gene_range[tid]))
            print('\t'.join(x for x in mRNA_tag))
            for line in gene_dff[tid]:
