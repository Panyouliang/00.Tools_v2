...
def out_p(chr_dict,gene_sort,old_id,name):
    ...
    out.write('#old-gene-id\told-transcript-id\tnew-gene-id\n')
    for chr,gid_lis in gene_sort.items():
        for gid in gid_lis:
            gnum += 1
            iso = 0
            geneID = str(gnum).zfill(6)
            out.write(gid[0]+'\t'+old_id[gid[0]]+'\t'+name+'G'+geneID+'\n')
            ...
            for tid,tvalue in chr_dict[chr][gid[0]].items():
                ...
                for line in tvalue:
                    if line[2] == 'mRNA':
                        print('\t'.join(x for x in line[0:8]),'ID='+name+'T'+geneID+'t'+str(iso)+';Parent='+name+'G'+geneID+';',sep='\t')
                    if line[2] == 'UTR5' or line[2] == 'five_prime_UTR':
                        utr5 += 1
                        print('\t'.join(x for x in line[0:8]),'ID='+name+'T'+geneID+'t'+str(iso)+'utr5.'+str(utr5)+';Parent='+name+'M'+geneID+'t'+str(iso)+';',sep='\t')
                    if line[2] == 'exon':
                        exon += 1
                        print('\t'.join(x for x in line[0:8]),'ID='+name+'T'+geneID+'t'+str(iso)+'exon.'+str(exon)+';Parent='+name+'M'+geneID+'t'+str(iso)+';',sep='\t')
                    if line[2] == 'CDS':
                        cds += 1
                        print('\t'.join(x for x in line[0:8]),'ID='+name+'T'+geneID+'t'+str(iso)+'cds.'+str(cds)+';Parent='+name+'M'+geneID+'t'+str(iso)+';',sep='\t')
                    if line[2] == 'UTR3' or line[2] == 'three_prime_UTR':
                        utr3 += 1
                        print('\t'.join(x for x in line[0:8]),'ID='+name+'T'+geneID+'t'+str(iso)+'utr3.'+str(utr3)+';Parent='+name+'M'+geneID+'t'+str(iso)+';',sep='\t')
