...
parser.add_argument('-Iso_gff',  metavar='genome file', type=str, required=False, help='Please input the stringtie assemble file of ISO-seq(gff format)')
    ...
    for line in f.readlines():
        if line.strip():
            if line[0] != '#':
                ...
                if line[2] == 'mRNA':
                    gidx, tidx = line[8].find('Parent') if 'Parent' in line[8] else line[8].find('gene_id'), line[8].find('ID')
    ...
    for gid,cds_interval in gene_dict.items():
        ...
        if gene2Iso[gid] in cdsgID2exongID.keys():
            for exon_ptid in cdsgID2exongID[gene2Iso[gid]]:
                if exon_ptid in trans_dict.keys():
...
def out_print(all_gene,gene_pos,UTR5_dict,UTR3_dict,genoUTR5,genoUTR3,gene_tag):
    for gid,m_value in all_gene.items():
        ...
        if gid in UTR5_dict.keys():
            ...
            if gid in genoUTR5.keys():
                for piece in genoUTR5[gid]:
                    utr5 += 1
                    exon += 1
                    print('\t'.join(x for x in gene_tag[gid][0:2]),'UTR5',piece[0],piece[1],'\t'.join(x for x in gene_tag[gid][5:8]),'Parent='+tid+';',sep='\t')
        ...
        if gid in UTR3_dict.keys():
            ...
            if gid in genoUTR3.keys():
                for piece in genoUTR3[gid]:
                    utr3 += 1
                    exon += 1
                    print('\t'.join(x for x in gene_tag[gid][0:2]),'UTR3',piece[0],piece[1],'\t'.join(x for x in gene_tag[gid][5:8]),'Parent='+tid+';',sep='\t')

def main():
    gff1,sp = args.core_gff,args.sp
    gff2,gff3 = args.Iso_gff,args.ngs_gff

    gene_dict,gene2trans,all_gene,gene_tag,gemoma_utr5,gemoma_utr3,gemoma_utr5_lis,gemoma_utr3_lis = read_gff_CDS(gff1)

    if gff2 is not None and gff3 is None:
        ...
        out_print(all_gene,gene_pos_iso,UTR5_iso_dict,UTR3_iso_dict,gemoma_utr5,gemoma_utr3,gene_tag)

    if gff3 is not None and gff2 is None:
        ...
        out_print(all_gene,gene_pos_ngs,UTR5_ngs_dict,UTR3_ngs_dict,gemoma_utr5,gemoma_utr3,gene_tag)
    ...
    if gff2 and gff3:
        ...
        for gid,cds_v in all_gene.items():
            ...
            for tid,coding in cds_v.items():
                ggidx = coding[0][8].find('Parent') if 'Parent' in coding[0][8] else coding[0][8].find('gene_id')
