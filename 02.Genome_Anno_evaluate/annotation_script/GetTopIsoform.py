        ...
        if line.strip():
            if line[0] != '#':
                ...
                if line[2] == 'gene':
                    ...
                    if line[8].find('Parent')>0:
                        ggidx = line[8].find('Parent')
                    else:
                        if line[8].find('gene_id')>0:
                            ggidx = line[8].find('gene_id')

                    ggid = line[8][ggidx:].split(';')[0].split('=')[1]+' '+line[6]
    ...
    for k,v in all_gff_file.items():
        for m,n in v.items():
            if k.endswith('+'):
                ...
                model = sorted(n[1:], key = (lambda x:int(x[3])),reverse=True)
                model.insert(0,n[0])
                v[m] = model
...
def main():
    ...
    out2 = open(name+'.top.iso.coding.gff','w')
